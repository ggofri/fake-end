...
jest.mock('commander');
jest.mock('chalk', () => ({
  red: jest.fn((...args: any[]) => `RED: ${args.map(arg => arg instanceof Error ? arg.toString() : arg).join(' ')}`)
}));
jest.mock('../handlers/run');
jest.mock('@/mock-generation');
...
describe('CLI Index', () => {
  ...
  describe('Run Command', () => {
    ...
    it('should call startServer with parsed options', async () => {
      ...
      expect(mockStartServer).toHaveBeenCalledWith({ port: 3000, mockDir: 'test_dir', verbose: true, noCache: false, dynamicMocks: false, watch: false });

      await runActionCallback({ port: '4000', dir: 'mock_server' });
      expect(mockStartServer).toHaveBeenCalledWith({ port: 4000, mockDir: 'mock_server', verbose: false, noCache: false, dynamicMocks: false, watch: false });
    });
    ...
    it('should handle startServer errors', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('RED: Error starting server'));
  ...
  describe('Generate Command', () => {
    ...
    it('should handle generateMockFromCurl errors', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('RED: Error generating mock'));
      expect(processExitSpy).toHaveBeenCalledWith(1);
    });
  });
